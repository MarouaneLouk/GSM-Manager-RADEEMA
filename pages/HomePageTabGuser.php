...
<div class="table-container">
    <h2 style="margin-left: 6px;">Gestion Utilisateurs</h2>
    <div class="table-header">
        <div class="firstdiv">
                    ...
                    <div class="popup-body">
                        <form id="filter-form">
                            <div class="form-group">
                                ...
                                <?php
                                    $orderBySelector = new UsersView();
                                    $orderBySelector = $orderBySelector->showorderBySelector($_SESSION['Id_User']);
                                ?>
                            </div>
                            <div class="form-group">
            ...
            <div class="search-div" style="display: inline;">
                <input class="searchbar" type="text" id="searchTable" placeholder="Search Users by Name, Password, ...">
        ...
        <div id="createPopup" class="popup">
            <div class="popup-content">
                ...
                <form action="../php/createUser.php" method="post">
                    ...
                    <input class="addWeight" type="text" id="nomUser" name="nomUser" minlength=3 maxlength=100 required>
                    <label for="prenomUser"><span style="color:red">*</span>Prénom Utilisateur:</label>
                    <input class="addWeight" type="text" id="prenomUser" name="prenomUser" minlength=3 maxlength=100 required>
                    <label for="modePasse"><span style="color:red">*</span>Mode De Passe:</label>
                    <input class="addWeight" type="text" id="modePasse" name="motPasse" minlength=8 maxlength=100 required>
    ...
    <?php
        if(isset($_SESSION["userError"])){
            ...
            echo '<p id="message-text">' . $_SESSION["userError"] . '</p>';
            echo '</div>';
        ...
        if (isset($_SESSION['deleteUserError'])) {
            echo '<div id="message-container" class="hidden">';
            echo '<p id="message-text">' . $_SESSION['deleteUserError'] . '</p>';
            echo '</div>';
            unset($_SESSION['deleteUserError']);
        }
        if (isset($_SESSION['modifyUserError'])) {
            echo '<div id="message-container" class="hidden">';
            echo '<p id="message-text">' . $_SESSION['modifyUserError'] . '</p>';
            echo '</div>';
            unset($_SESSION['modifyUserError']);
        }
    ?>
    <div class="table" id="table-container">
    <form id="modifyForm" action="../php/deleteModifyUser.php" method="POST">
        <table id="example-table">
            <?php
                $checkUser = new usersContr();
                $checkUser = $checkUser->checkUserByIdException($_SESSION['Id_User']);
            ?>
            <thead>
                <tr>
                    <?php
                        $tHeaderContent = new UsersView();
                        $tHeaderContent = $tHeaderContent->viewtHeaderContent($_SESSION['Id_User']);
                    ?>
                </tr>
            </thead>
            <tbody id="table-body">
                <?php
                    ...
                    $nbrowtab = $userTable->showUserTable($_SESSION['Id_User']);
                ?>
            </tbody>
        </table>
    </form>
